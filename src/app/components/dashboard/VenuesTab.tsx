...
type VType = "Marriage Halls" | "Convention Centers" | "Resorts" | "Hotels" | "Outdoor Venues";

const VENUES: Record<VType, { name: string; location: string; capacity: string; price: string; rating: number; img: string; tags: string[]; avail: string }[]> = {
  "Marriage Halls": [
...
const TABS: VType[] = ["Marriage Halls", "Convention Centers", "Resorts", "Hotels", "Outdoor Venues"];
...
export default function VenuesTab() {
  const [active, setActive] = useState<VType>("Marriage Halls");
  const [dbVenues, setDbVenues] = useState<Record<VType, any[]>>({
    "Marriage Halls": [],
    "Convention Centers": [],
    "Resorts": [],
    "Hotels": [],
  ...
  const isNearUser = (venueLocation: string) => {
    ...
    const fetchUserProfileAndVenues = async () => {
      try {
        ...
        if (!error && data && data.length > 0) {
          const grouped: Record<VType, any[]> = {
            "Marriage Halls": [],
            "Convention Centers": [],
            "Resorts": [],
            "Hotels": [],
