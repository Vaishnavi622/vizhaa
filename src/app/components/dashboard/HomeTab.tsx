...
import { ArrowRight, Star, Phone, Mail, MapPin, Sparkles, TrendingUp, Gift, Award, CheckCircle, Award as Crown } from "lucide-react";
...
export default function HomeTab({ onNavigate, onEventClick }: Props) {
  ...
  const loadLoyaltyData = async () => {
    try {
      ...
      const claimedSet = new Set<string>((cData || []).map(c => c.scheme_id));
  ...
  const handleClaimReward = async (schemeId: string, schemeName: string, schemeRewardPoints: number) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
  ...
  return (
    <div style={{ padding: "0 0 3rem" }}>
      ...
      <div style={{ padding: "2.5rem 2rem 0" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem" }}>
              ...
              <div style={{ background: "linear-gradient(135deg, #1a1408, #14100a)", border: `1px solid ${G.gold}33`, borderRadius: "12px", padding: "1.5rem", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                ...
                <p style={{ color: G.muted, fontSize: "0.8rem", margin: 0 }}>
                  Active Bookings: <strong>{bookingsCount}</strong> · Claim rewards below as you reach new milestones!
                </p>
              </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1.25rem", marginBottom: "3rem" }}>
          {(liveReviews.length > 0 ? liveReviews : TESTIMONIALS).map((t: any, i: number) => {
