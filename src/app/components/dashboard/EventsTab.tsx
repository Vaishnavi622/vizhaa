...
import { Users, Clock, ChevronLeft, CheckCircle, ArrowRight, X, Car, BedDouble, Camera, Sparkles, ChevronDown, Upload, Image as ImgIcon, MapPin, Palette, Utensils, Music, Mail, Gift } from "lucide-react";
...
type Category = "Marriage Events" | "Birthday Celebrations" | "Family Celebrations";
type EventCard = { name: string; desc: string; img: string; duration: string; guests: string; price: string };
type ECard = EventCard;

/* ─── UI ATOMS ─── */

function ModalWrap({ onClose, onSubmit, children }: { onClose: () => void; onSubmit?: (e: React.FormEvent<HTMLFormElement>) => void; children: React.ReactNode }) {
  return (
...
function TickCheck({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
...
function Input({ placeholder, type = "text", name, required }: { placeholder?: string; type?: string; name?: string; required?: boolean }) {
...
function UploadBtn({ label = "Upload Reference Image", name }: { label?: string; name?: string }) {
...
/* ─── COMMON EVENT FORM ─── */
function CommonEventForm({ hideVenueLocation, hideGuests }: { hideVenueLocation?: boolean; hideGuests?: boolean }) {
  return (
    <div>
...
/* ─── VENUE BOOKING MODAL ─── */
function VenueBookingModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── CATERING MODAL ─── */
const CATERING_MENU: Record<string, Record<string, { Starter: string[]; MainCourse: string[]; Dessert: string[] }>> = {
  Continental: {
...
const SECTION_DOTS: Record<string, string> = { Starter: "#60a5fa", MainCourse: G.gold, Dessert: "#f9a8d4" };

function CateringModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
        ...
        {selectedCuisine && menuData && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} style={{ overflow: "hidden" }}>
            <div style={{ background: G.surface2, border: `1px solid ${G.border}`, borderRadius: "10px", padding: "1.25rem", marginBottom: "1.25rem" }}>
              ...
              {(["Starter","MainCourse","Dessert"] as const).map(sec => (
...
/* ─── MAKEUP MODAL ─── */
function MakeupModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── PHOTOGRAPHY MODAL ─── */
function PhotographyModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
  const [indoor, setIndoor] = useState(false);
  const [outdoor, setOutdoor] = useState(false);
  const [drone, setDrone] = useState(false);
  const [coverage, setCoverage] = useState<string[]>([]);
  const [videoPackages, setVideoPackages] = useState<string[]>([]);
...
/* ─── DECORATION MODAL ─── */
function DecorationModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── INVITATION MODAL ─── */
function InvitationModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
  ...
  const togglePhoto = (i: number) => setSelectedPhotos(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i]);
...
/* ─── CAKE MODAL ─── */
function CakeModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── ENTERTAINMENT MODAL ─── */
function EntertainmentModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── RETURN GIFTS MODAL ─── */
function ReturnGiftsModal({ eventName, onClose, onSubmit }: { eventName: string; onClose: () => void; onSubmit: any }) {
...
function ImageSuggestionsModal({ serviceName, eventName, onClose, onSubmit }: { serviceName: string; eventName: string; onClose: () => void; onSubmit: any }) {
...
/* ─── MODAL TYPE ROUTING ─── */
type ModalType = "venue" | "catering" | "makeup" | "photography" | "decoration" | "cake" | "entertainment" | "return_gifts" | "invitation" | "images" | "general";

function getModalType(svc: string): ModalType {
  const s = svc.toLowerCase();
...
/* ─── EVENT SERVICES ─── */
const EVENT_SERVICES: Record<string, string[]> = {
...
export const getEventServices = (name: string) => {
...
export const getIncludedServices = (name: string) => {
  const clean = name.replace(/[^\w\s]/g, '').trim().toLowerCase();
...
/* ─── EVENTS DATA ─── */
const EVENTS: Record<Category, ECard[]> = {
  "Marriage Events": [
...
const FAMILY_PLANS: Record<string, {
  name: string;
  emoji: string;
  plans: {
    name: string;
...
const MARRIAGE_PLANS: Record<string, { name: string; emoji: string; plans: { name: string; badge: string; includes: string[] }[] }> = {
  "wedding": {
    name: "Wedding",
    emoji: "💍",
    plans: [
  ...
  // Add other marriage related categories as needed (reception, mehendi, haldi, sangeet, anniversary)
};

// Helper to get marriage key from event name
const getMarriageKey = (name: string) => {
...
const getFamilyCelebrationKey = (name: string) => {
  const clean = name.toLowerCase();
...
const FAMILY_GALLERIES: Record<string, string[]> = {
  "baby shower": [
...
const CATEGORIES: Category[] = ["Marriage Events","Birthday Celebrations","Family Celebrations"];
...
function EventDetail({ card, onBack }: { card: ECard; onBack: () => void }) {
  ...
  const subServicesNormalized = (subServices || []).map((sub: any) => {
        ...
        {servicesLoading ? (
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1.25rem", marginBottom: "2rem" }}>
              {subServicesNormalized.map((sub: { name: string; desc: string; img: string }) => {
...
export default function EventsTab({ initialEventName }: { initialEventName?: string }) {
  const [dbEvents, setDbEvents] = useState<Record<Category, ECard[]>>({
    "Marriage Events": [],
    "Birthday Celebrations": [],
    "Family Celebrations": []
  });
  const [loading, setLoading] = useState(true);
  const [activeCategory, setActiveCategory] = useState<Category>("Marriage Events");
  const [selectedCard, setSelectedCard] = useState<ECard | null>(null);
  ...
  useEffect(() => {
    const fetchDbEvents = async () => {
      try {
        ...
        if (!error && data && data.length > 0) {
          const grouped: Record<Category, ECard[]> = {
            "Marriage Events": [],
            "Birthday Celebrations": [],
            "Family Celebrations": []
          };
