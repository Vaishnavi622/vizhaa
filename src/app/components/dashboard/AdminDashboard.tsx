...
import {
  ...
  ThumbsDown, ChevronLeft, CreditCard as PayIcon, Package2, Wrench,
...
type AdminTab = "dashboard" | "services_mgmt" | "packages_mgmt" | "vendors_mgmt" | "bookings" | "reports" | "reviews" | "event_gallery" | "profile";
...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { id: "services_mgmt" as AdminTab, label: "Services",    icon: Scissors },
      { id: "packages_mgmt" as AdminTab, label: "Packages",    icon: Package2 },
      { id: "vendors_mgmt"  as AdminTab, label: "Vendors",     icon: Users },
      { id: "bookings"      as AdminTab, label: "Booking",     icon: ShoppingBag },
      { id: "reports"       as AdminTab, label: "Performance", icon: BarChart3 },
      { id: "reviews"       as AdminTab, label: "Feedback",    icon: Star },
      { id: "event_gallery" as AdminTab, label: "Event Gallery", icon: Images },
...
/* ─── shared helpers ─── */
const sc = (s: string) => s === "Confirmed" || s === "Completed" || s === "Paid" ? "#4ade80" : s === "Pending" ? G.gold : "#f87171";
const sb = (s: string) => s === "Confirmed" || s === "Completed" || s === "Paid" ? "rgba(74,222,128,0.1)" : s === "Pending" ? "rgba(201,168,76,0.1)" : "rgba(248,113,113,0.1)";
const SI = ({ s }: { s: string }) => s === "Confirmed" || s === "Completed" || s === "Paid" ? <CheckCircle size={12} /> : s === "Pending" ? <Clock size={12} /> : <XCircle size={12} />;
...
function ActionBtn({ icon: Icon, label, danger, onClick }: { icon: React.ElementType; label: string; danger?: boolean; onClick?: (e: React.MouseEvent) => void }) {
...
function AHead({ label, title }: { label: string; title: string }) {
  return (
...
/* ══════════════ CALENDAR COMPONENT ══════════════ */
function CalendarWidget({ bookings }: { bookings: any[] }) {
  ...
  const calendarEventsLookup: Record<string, { event: string; client: string; color: string }[]> = {};
  ...
  const eventsForDate = (d: Date) => calendarEventsLookup[getKey(d)] ?? [];
...
function DashboardPanel({ bookings, profiles }: { bookings: any[]; profiles: any[] }) {
...
/* ══════════════ BOOKINGS PANEL ══════════════ */
function BookingsPanel({ bookings, onReload }: { bookings: any[]; onReload: () => void }) {
            ...
            {selectedBooking.services && selectedBooking.services.length > 0 && (
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                  {selectedBooking.services.map((s: string) => <span key={s} style={{ background: "rgba(201,168,76,0.08)", border: `1px solid ${G.border}`, color: G.muted, padding: "3px 10px", borderRadius: "99px", fontSize: "0.72rem" }}>{s}</span>)}
                </div>
              </div>
            )}
...
type SubItemDetail = { vendorName?: string; description: string; images: string[] };
...
function ServicesMgmtPanel() {
  ...
  const [subEditKey, setSubEditKey] = useState<string>("");
  const [subEditData, setSubEditData] = useState<SubItemDetail>({ vendorName: "", description: "", images: [] });
  const [subItemDetails, setSubItemDetails] = useState<Record<string, SubItemDetail>>({});

  // Multiple vendors management states
  const [editingVendors, setEditingVendors] = useState<any[]>([]);
  const [activeVendorIndex, setActiveVendorIndex] = useState<number | null>(null);
  const [vendorForm, setVendorForm] = useState<{ vendorName: string; description: string; phone: string; email: string; price: string; images: string[] }>({ vendorName: "", description: "", phone: "", email: "", price: "", images: [] });

  // Sub-items add/edit modal states
  const [subItemModalOpen, setSubItemModalOpen] = useState(false);
  const [currentSubItem, setCurrentSubItem] = useState<{ name: string; desc: string; img: string; index?: number } | null>(null);
  const [subItemServiceId, setSubItemServiceId] = useState<string | null>(null);
  const [subItemUploading, setSubItemUploading] = useState(false);
  const [subItemMediaPreview, setSubItemMediaPreview] = useState<string | null>(null);

  const [uploadingMedia, setUploadingMedia] = useState(false);
  const [mediaPreview, setMediaPreview] = useState<string | null>(null);
  const [mediaTypeField, setMediaTypeField] = useState<string>("image");

  const openSubEdit = (catName: string, svcName: string) => {
  ...
  const saveSubEdit = async () => {
    try {
      ...
      if (allImages.length > 0) {
        const getGalleryCategory = (categoryName: string) => {
        ...
        supabase
          ...
          .then(({ data: existingGallery }) => {
            const existingUrls = new Set((existingGallery || []).map((item: any) => item.url));
            const newImagesToInsert: { url: string; vendorName: string }[] = [];
            editingVendors.forEach(v => {
              if (v.images) {
                v.images.forEach((img: string) => {
  ...
  const loadSubItemDetails = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select("*");
      if (!error && data) {
        const mapping: Record<string, SubItemDetail> = {};
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const reviewsRaw = formData.get("reviews_count") as string;
    const reviews_count = reviewsRaw ? parseInt(reviewsRaw) : 150;

    const media_type = mediaTypeField;
    const imgUrlInput = formData.get("img_url") as string;
  ...
  const handleSaveSubItem = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!subItemServiceId) return;

    const formData = new FormData(e.currentTarget);
    const name = formData.get("sub_name") as string;
    const desc = formData.get("sub_desc") as string;
    const imgUrlInput = formData.get("sub_img_url") as string;
  ...
  const handleEditSubItem = (serviceId: string, item: any, index: number) => {
  ...
  const handleDeleteSubItem = async (serviceId: string, index: number) => {
    if (!window.confirm("Are you sure you want to delete this service?")) return;
    const service = services.find(s => s.id === serviceId);
    if (!service) return;

    const items = (service.items || []).filter((_: any, idx: number) => idx !== index);
  ...
  const handleSubItemFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
...
function VendorsPanel() {
  const [subItemDetails, setSubItemDetails] = useState<Record<string, any>>({});
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => { loadSubItemDetails(); }, []);

  type VendorEntry = {
    id: string; name: string; description: string;
    phone: string; email: string; price: string;
    service: string; category: string; images: string[];
    originalIndex: number;
    dbKey: string;
  };

  const getAllVendors = (): VendorEntry[] => {
    const list: VendorEntry[] = [];
    let globalIdx = 0;
    Object.entries(subItemDetails).forEach(([key, detail]: [string, any]) => {
      ...
      if (detail?.description) {
        try {
          ...
          vendors.forEach((v: any, vi: number) => {
            globalIdx++;
            list.push({
...
/* ══════════════ PERFORMANCE PANEL ══════════════ */
function ReportsPanel({ bookings = [], profiles = [] }: { bookings?: any[]; profiles?: any[] }) {
          ...
          {totalPayment === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthlyRevenue}>
                ...
                <Tooltip contentStyle={tooltipStyle} formatter={(value: number) => [`₹${value}K`, "Revenue"]} />
...
/* ══════════════ REVIEWS PANEL ══════════════ */
function StarRow({ n, filled }: { n: number; filled: number }) {
  return (
...
function ReviewsPanel({ reviews, onReload }: { reviews: any[]; onReload: () => void }) {
  ...
  const servicePerformance = SERVICES_LIST.map(service => {
    ...
    const avg = count > 0 ? matching.reduce((sum, r) => sum + r.service_rating!, 0) / count : 0;
    return {
      service,
  ...
  const supervisorPerformance = SUPERVISORS_LIST.map(supervisor => {
    ...
    const count = matching.length;
    const avg = count > 0 ? matching.reduce((sum, r) => sum + r.supervisor_rating!, 0) / count : 0;
    return {
      supervisor,
      events: count,
      rating: count > 0 ? `⭐ ${avg.toFixed(1)}` : "⭐ 5.0"
  ...
  const handleReply = async (reviewId: string, currentReply: string) => {
    const replyText = prompt("Enter your reply as Admin:", currentReply || "");
                ...
                <div style={{ height: "160px", background: "#1a1610", position: "relative", overflow: "hidden" }}>
                  {isVideo ? (
                    ...
                    <img src={item.url} alt={item.caption} style={{ width: "100%", height: "100%", objectFit: "cover" }}
                      onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1587271407850-8d438ca9fdf2?w=400&h=300&fit=crop"; }} />
                  )}
                </div>
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (adminId && profilesData) {
        const adminProf = profilesData.find((p: any) => p.id === adminId);
        if (adminProf?.full_name) {
          setAdminName(adminProf.full_name);
        }
      }

      let finalProfiles = (profilesData || []).filter((p: any) => {
      ...
      const profilesMap = new Map(finalProfiles.map((p: any) => [p.id, p]));

      const processedBookings = finalBookings.map((b: any) => ({
        ...b,
