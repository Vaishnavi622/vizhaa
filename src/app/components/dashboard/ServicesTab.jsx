...
const getIcon = (name) => G_ICONS[name] || Sparkles;

const resolveDetailKey = (categoryName, subName) => {
...
export default function ServicesTab({ initialEventType } = {}) {
  ...
  const [advancePaymentOption, setAdvancePaymentOption] = useState('50');
  ...
  const [editingSubServiceParent, setEditingSubServiceParent] = useState(null);
  ...
  const handleDeleteService = async (svcName) => {
  ...
  const handleEditSubService = (parentSvcName, subService) => {
  ...
  const handleDeleteSubService = async (parentSvcName, subSvcName) => {
    if (!window.confirm(`Are you sure you want to delete the sub-service "${subSvcName}"?`)) return;
    const parent = services.find(s => s.name === parentSvcName);
    if (parent) {
      const updatedItems = (parent.items || []).filter((item) => 
  ...
  useEffect(() => {
    ...
    async function fetchRecs(svcName) {
  ...
  const handleBadgeClick = (subName, categoryName) => {
          ...
          (() => {
            ...
            if (activeSubService) {
              ...
              const handlePrev = () => {
                ...
                const currentIndex = subItems.findIndex((it) => (typeof it === "string" ? it : it.name) === subName);
                if (currentIndex !== -1) {
