...
export function AdminPackagesPanel() {
  ...
  const [currentPkg, setCurrentPkg] = useState<EventPackage | null>(null); // null means adding a new package
  ...
  const [platinumFeatures, setPlatinumFeatures] = useState<string[]>([]);
  ...
  const handleAddFeature = (tier: "silver" | "gold" | "platinum") => {
  ...
  const handleRemoveFeature = (tier: "silver" | "gold" | "platinum", index: number) => {
    if (tier === "silver") {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const updatedTiers: PackageTier[] = [
      {
        name: "Silver Package",
        price: silverPrice,
