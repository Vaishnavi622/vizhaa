...
type BTab = "Active Bookings" | "Booking History" | "Payment Status";

const statusColor = (s: string) => s === "Confirmed" || s === "Paid" || s === "Completed" ? "#4ade80" : s === "In Progress" || s === "Pending" ? G.gold : "#f87171";
const statusBg   = (s: string) => s === "Confirmed" || s === "Paid" || s === "Completed" ? "rgba(74,222,128,0.1)" : s === "In Progress" || s === "Pending" ? "rgba(201,168,76,0.1)" : "rgba(248,113,113,0.1)";
const StatusIcon = ({ s }: { s: string }) => s === "Confirmed" || s === "Paid" || s === "Completed" ? <CheckCircle size={13} /> : s === "In Progress" || s === "Pending" ? <Clock size={13} /> : <XCircle size={13} />;

const TABS: BTab[] = ["Active Bookings", "Booking History", "Payment Status"];
...
export default function BookingsTab() {
  const [active, setActive] = useState<BTab>("Active Bookings");
  ...
  const handleOpenFeedback = (b: any | null) => {
                  ...
                  <motion.div key={b.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                    ...
                    {b.services && b.services.length > 0 && (
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                        {b.services.map((s: string) => <span key={s} style={{ background: "rgba(201,168,76,0.08)", border: `1px solid ${G.border}`, color: G.muted, padding: "3px 10px", borderRadius: "99px", fontSize: "0.72rem" }}>{s}</span>)}
                      </div>
                    )}
                  ...
                  historyBookings.map((b, i) => (
                    <motion.div key={b.id} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                      ...
                      <div>
                        ...
                        <p style={{ color: G.muted, fontSize: "0.75rem" }}>{b.id.substring(0, 8).toUpperCase()} · {b.venue || "TBD"}</p>
                ...
                payments.map((p, i) => (
                  <motion.div key={p.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p style={{ color: G.text, fontSize: "0.88rem", marginBottom: "2px" }}>{p.bookings?.event_name || "General Booking"} — {p.id.substring(0, 8).toUpperCase()}</p>
                        <p style={{ color: G.muted, fontSize: "0.72rem" }}>{formatDate(p.date)} · {p.method}</p>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
